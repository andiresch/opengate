...
doseFpath_IDD_d = str(output.get_actor(doseActorName_IDD_d).user_info.output).replace(
    ".mhd", "_dose.mhd"
)
doseFpath_IDD_d2w = str(
    output.get_actor(doseActorName_IDD_d2w).user_info.output
).replace(".mhd", "_doseToWater.mhd")
doseFpath_geoWater_d = str(
    output.get_actor(doseActorName_water_slab_insert_d).user_info.output
).replace(".mhd", "_dose.mhd")
doseFpath_geoWater_d2w = str(
    output.get_actor(doseActorName_water_slab_insert_d2w).user_info.output
).replace(".mhd", "_doseToWater.mhd")

doseFpath_geoSi_d = str(
    output.get_actor(doseActorName_entranceRegiont_d).user_info.output
).replace(".mhd", "_dose.mhd")
doseFpath_geoSi_d2w = str(
    output.get_actor(doseActorName_entranceRegiont_d2w).user_info.output
).replace(".mhd", "_doseToWater.mhd")
