...
class DosimetryActor(g4.GateDosimetryActor, gate.ActorBase):
    """
    DosimetryActor: compute a 3D edep/dose map for deposited
    ...
    def __init__(self, user_info):
        ...
        g4.GateDosimetryActor.__init__(self, user_info.__dict__)
    ...
    def __str__(self):
        u = self.user_info
        s = f'DosimetryActor "{u.name}": dim={u.size} spacing={u.spacing} {u.output} tr={u.translation}'
        return s
    ...
    def StartSimulationAction(self):
        ...
        try:
            ...
            gate.fatal(f"Error in the DosimetryActor {self.user_info.name}")
        ...
        if not self.first_run:
            gate.warning(f"Not implemented yet: DosimetryActor with several runs")
        ...
        if vol_type == "Image":
            ...
            if self.user_info.img_coord_system:
                gate.warning(
                    f'DosimetryActor "{self.user_info.name}" has '
    ...
    def EndSimulationAction(self):
        g4.GateDosimetryActor.EndSimulationAction(self)
