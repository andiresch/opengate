...
G4Mutex SetDosimetryActorPixelMutex = G4MUTEX_INITIALIZER;

GateDosimetryActor::GateDosimetryActor(py::dict &user_info)
...
void GateDosimetryActor::ActorInitialize() {
  if (fUncertaintyFlag) {
...
void GateDosimetryActor::BeginOfRunAction(const G4Run *) {
...
void GateDosimetryActor::SteppingAction(G4Step *step) {
  ...
  if (isInside) {
    // With mutex (thread)
    G4AutoLock mutex(&SetDosimetryActorPixelMutex);
...
void GateDosimetryActor::EndSimulationAction() {}
